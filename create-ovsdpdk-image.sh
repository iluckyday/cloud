...
LABEL debian
        LINUX /vmlinuz
        INITRD /initrd.img
        APPEND root=LABEL=debian-root quiet intel_iommu=on iommu=pt default_hugepagesz=1G hugepagesz=1G hugepages=4
EOF
...
apt install -y -o APT::Install-Recommends=0 -o APT::Install-Suggests=0 openvswitch-switch-dpdk
